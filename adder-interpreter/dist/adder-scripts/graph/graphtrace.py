...
CreateConsole( version + " (graphtrace)" )
...
def add_library_trace( lst, lib, func ):
    mod = LoadLibrary( lib )
    proc = GetProcAddress( ptr(mod), func )
    if proc == NULL:
...
usage = "usage: %prog idc-file output-file"
...
if len(args) == 0:
    ...
    sys.exit(1)
if len(args) < 2:
    print ("need to specify a idc-file and a output-file")
    raw_input("Press Enter To Exit")
